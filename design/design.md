...
3. **Routing decision**:
   ...
   - `"media"` → tool_node (directly invokes analyze_image or analyze_video)
   - `"general"` → agent_node (main reasoning loop with tool calling)
...
- **Code execution has no network access**: Docker sandbox runs with `--network none` by design. Only Python standard library is available. Pre-built image with common packages is a future improvement.
...
### Built
- Input Router (keyword pattern matching — 50+ patterns across code/media/general)
